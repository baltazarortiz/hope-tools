...
def soft_reset(exe_path, openocd_log_file, gdb_log_file):
    logger.info("Soft resetting FPGA")
    openocd_proc = start_openocd(log_file=openocd_log_file)
    gdb_reset(exe_path, gdb_log_file)
...
def gdb_reset(exe_path, log_file=None):
    if log_file:
...
def runSim(exe_path, run_dir, policy_dir, pex_path, runtime, rule_cache,
    ...
    if extra_args.bitstream:
        ...
        if not soft_reset(exe_path, openocd_log_file, gdb_log_file):
            logger.error('''
